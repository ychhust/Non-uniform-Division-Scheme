...
%基线排列方式单位为波长：
% 最小间距
sys_param.min_spacing = 0.5;    
%接收机温度，即TB。单位：K
sys_param.T_rec = 300;

%% 场景定义
STM_param.extentpoint_simu = 1;      %是否仿真，0表示不仿真，1表示仿真
STM_param.extentpoint_place_start = [-20 10];       %来波方向起始位置，度
STM_param.extentpoint_power = [250 50];    %源的能量（K）
STM_param.extentpoint_place_end = [20 20];       %来波方向终止位置，度
...
%积分运算
delta_x=10;
F=@(x)exp(1i*2*pi*x*delta_x*min_spacing);
...
V1(i/div)=real(visibility(30+delta_x));
V2(i/div)=imag(visibility(30+delta_x));
coef_matrix(:)=[];
    end
...
figure(2)
subplot(2,1,1)
plot((1:N)*div,log(abs(V1-Vr)),'LineWidth',2)
xlabel('场景分隔点数')
ylabel('可见度实部相对误差(log)')
subplot(2,1,2)
plot((1:N)*div,log(abs(V2-Vi)),'LineWidth',2)
xlabel('场景分隔点数')
ylabel('可见度虚部相对误差(log)')

figure(3)
plot(Fov0,T,'-','LineWidth',2)
xlabel('无混叠视场范围')
ylabel('场景亮温')
title('场景反演结果')
axis square
